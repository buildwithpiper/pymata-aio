 ...
 * According to Firmata.h, 0x00-0x0F are reserved for user-defined commands.
 */
#define HID_GET 0x00
#define HID_SET 0x01
...
#define HID_BUTTON_DOWN 7
#define HID_BUTTON_LEFT 8
...
#define HID_SETTING_JS_INVERTED 102
...
const uint8_t hidJoystickPitchAxis = A0;                   // joystick Y axis
const uint8_t hidJoystickRange = 12;                       // joystick output range
const uint8_t hidJoystickThreshold = hidJoystickRange / 4; // joystick resting threshold
const uint8_t hidJoystickCenter = hidJoystickRange / 2;    // joystick resting position value
const uint8_t hidResponseDelay = 5;                        // HID response delay in ms
bool hidEnabled = false;
int hidMillis = 0;

/* configurable HID event <-> action mappings */
char hidMappingDPadUp = MOUSE_RIGHT;
char hidMappingDPadDown = 'w';
char hidMappingDPadLeft = MOUSE_LEFT;
char hidMappingDPadRight = ' ';
char hidMappingJoystickPress = KEY_LEFT_SHIFT;
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  case HID_GET:
    config = argv[0];
    Firmata.write(START_SYSEX);
    Firmata.write(HID_RESPONSE);
    Firmata.write(config);
    Firmata.write((byte)getHIDConfig(config));
    Firmata.write(END_SYSEX);
    break;
  case HID_SET:
    config = argv[0];
    byte value = argv[1];
    setHIDConfig(config, value);
    break;
  }
}
...
void loop()
{
  ...
  if (hidEnabled && currentMillis - hidMillis > hidResponseDelay)
  {
    hidMillis += hidResponseDelay;
    doHID();
  }
...
void toggleHID(bool enabled)
{
  ...
  {
    // enable joystick
    pinMode(hidJoystickYawAxis, INPUT);
    pinMode(hidJoystickPitchAxis, INPUT);
    pinMode(HID_BUTTON_JOYSTICK, INPUT_PULLUP);
    Mouse.begin();

    // enable d-pad
    pinMode(HID_BUTTON_UP, INPUT_PULLUP);
    pinMode(HID_BUTTON_DOWN, INPUT_PULLUP);
    pinMode(HID_BUTTON_LEFT, INPUT_PULLUP);
    pinMode(HID_BUTTON_RIGHT, INPUT_PULLUP);
  }
  else
  {
    Mouse.end();
...
void checkButtonsForChanges()
{
  ...
  doButton(HID_BUTTON_LEFT, hidMappingDPadLeft);
  doButton(HID_BUTTON_RIGHT, hidMappingDPadRight);
  doButton(HID_BUTTON_JOYSTICK, hidMappingJoystickPress);
}
...
int readAxis(int thisAxis)
{
  ...
  int scaled = map(reading, 0, 1023, 0, hidJoystickRange);

  // if the output reading is outside from the rest position threshold, use it:
  int distance = scaled - hidJoystickCenter;

  if (abs(distance) < hidJoystickThreshold)
  {
    distance = 0;
  }
...
byte getHIDConfig(uint8_t config)
{
  ...
  case HID_ENABLED:
    return hidEnabled;
  case HID_BUTTON_UP:
    return hidMappingDPadUp;
  case HID_BUTTON_DOWN:
    return hidMappingDPadDown;
  case HID_BUTTON_LEFT:
    return hidMappingDPadLeft;
  case HID_BUTTON_RIGHT:
    return hidMappingDPadRight;
  case HID_BUTTON_JOYSTICK:
    return hidMappingJoystickPress;
  case HID_SETTING_JS_SENSITIVITY:
    return hidSettingJoystickSensitivity;
  case HID_SETTING_JS_INVERTED:
    return hidSettingJoystickInverted;
  }
  return -1;
}

void setHIDConfig(uint8_t config, byte value)
{
  switch (config)
  {
  case HID_ENABLED:
    toggleHID((bool)value);
    return;
  case HID_BUTTON_UP:
    hidMappingDPadUp = (char)value;
    return;
  case HID_BUTTON_DOWN:
    hidMappingDPadDown = (char)value;
    return;
  case HID_BUTTON_LEFT:
    hidMappingDPadLeft = (char)value;
    return;
  case HID_BUTTON_RIGHT:
    hidMappingDPadRight = (char)value;
    return;
  case HID_BUTTON_JOYSTICK:
    hidMappingJoystickPress = (char)value;
    return;
  case HID_SETTING_JS_SENSITIVITY:
    hidSettingJoystickSensitivity = (int8_t)value;
    return;
  case HID_SETTING_JS_INVERTED:
    hidSettingJoystickInverted = (bool)value;
    return;
  }
}
